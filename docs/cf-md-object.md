...
## Пакетное создание (`add-md-object-batch`)

Набор объектов создаётся одним вызовом `apply-mutation` с описанием в `payloadJson`:

```json
{
  "op": "add-md-object-batch",
  "configurationXml": "C:/проект/src/cf/Configuration.xml",
  "schemaVersion": "V2_20",
  "payloadJson": "[{\"type\":\"CATALOG\",\"name\":\"Валюты\",\"synonymRu\":\"Валюты организации\"},{\"type\":\"DOCUMENT\",\"name\":\"ЗаказКлиента\"}]"
}
```

Виды объектов те же, что у `--type` в `add-md-object`; `synonymRu`, как и там, поддерживается для
справочника. В ответ приходят имена созданных объектов вида `Catalog.Валюты`.

Набор создаётся целиком или не создаётся вовсе. До записи проверяются виды, имена, повторы внутри
набора и то, что таких объектов ещё нет в выгрузке: одиночное добавление существующий объект
перезапишет молча, а для набора это половина работы, которую потом не отличить от целой. Если
ошибка всё же случилась в середине, созданные файлы удаляются, а состав восстанавливается из
снимка `Configuration.xml`, сделанного до начала.
