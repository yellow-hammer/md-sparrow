...
Заданное имя - обязательство: если объект с таким именем уже есть или путь объекта занят,
операция отказывается и ничего не создаёт. Подбор первого свободного имени вида `Справочник1` -
отдельная операция (`--auto-name`), она для того и есть.
